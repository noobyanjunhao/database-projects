...
def bill_payment_dashboard() -> str:
    """Display the bill payment dashboard."""
    db = get_db()
...
def create_bill(unit_id: int) -> Tuple[str, int]:
    ...
    db.execute(
        """
            INSERT INTO Bill (lease_id, billing_month, rent_amount, other_charges,
...
def bill_detail(bill_id: int) -> Union[str, Tuple[str, int]]:
    """Show the details of a specific bill."""
    db = get_db()
    bill = db.execute(
        """
        SELECT B.sent_at, B.billing_month, B.rent_amount, B.other_charges, 
            B.balance_used, B.total_amount, T.full_name, 
            A.unit_number, A.id AS apartment_id
        FROM Bill B
...
def payment_detail(payment_id: int) -> Union[str, Tuple[str, int]]:
    ...
    return render_template(
        "payment_detail.html", payment=payment, apartment_id=payment["apartment_id"]
    )
