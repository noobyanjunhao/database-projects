...
def export_units() -> Any:
    """Export unit data and return as an Excel file."""
    db = get_db()
    units = db.execute(
        """
        ...
        data.append(
            {
                ...
                "Monthly Rent": (
                    u["monthly_rent"] if u["ownership_type"] != "sold" else None
                ),
                "Rent End Date": (
                    u["end_date"] if u["ownership_type"] != "sold" else None
                ),
            }
        )
